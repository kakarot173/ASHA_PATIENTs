                                ...
                                completion: { [weak self] error in
                                    ...
                                    if let error = error {
                                        if error._code == QBResponseStatusCode.unAuthorized.rawValue {
                                            // Clean profile
                                            Profile.clearProfile()
                                            DispatchQueue.main.asyncAfter(deadline: DispatchTime.now() + .seconds(1)) { AppDelegate.shared.chatRootViewController.showLoginScreen()
                                            }
                                        }
                                        ...
                                        //did Login action
                                        DispatchQueue.main.asyncAfter(deadline: DispatchTime.now() + .seconds(1)) {
                                            AppDelegate.shared.chatRootViewController.goToDialogsScreen()
                                        }
                                    }
        })
    }
