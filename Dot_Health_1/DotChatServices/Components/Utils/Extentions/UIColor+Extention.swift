...
public convenience init(chatHex: Int, alpha: CGFloat = 1.0) {
    let red = CGFloat((chatHex & 0xFF0000) >> 16) / 255.0
    let green = CGFloat((chatHex & 0xFF00) >> 8) / 255.0
    let blue = CGFloat((chatHex & 0xFF)) / 255.0

    self.init(red: red, green: green, blue: blue, alpha: alpha)
}

public convenience init(chatHex string: String, alpha: CGFloat = 1.0) {
