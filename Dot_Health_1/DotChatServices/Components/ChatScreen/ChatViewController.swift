...
class ChatViewController: UIViewController {
    ...
    override func viewWillAppear(_ animated: Bool) {
        ...
        let updateConnectionStatus: ((_ status: NetworkConnectionStatus) -> Void)? = { [weak self] status in
            ...
            if notConnection == true {
                self?.showAlertView(ChatLoginConstant.checkInternet, message: ChatLoginConstant.checkInternetMessage)
            }
        }
    ...
    private func registerCells() {
        ...
        ChatDateCell.registerForReuse(inView: collectionView ?? UICollectionView())
        ChatNotificationCell.registerForReuse(inView: collectionView ?? UICollectionView())
        ChatOutgoingCell.registerForReuse(inView: collectionView ?? UICollectionView())
        ChatIncomingCell.registerForReuse(inView: collectionView ?? UICollectionView())
        ChatAttachmentIncomingCell.registerForReuse(inView: collectionView ?? UICollectionView())
        ChatAttachmentOutgoingCell.registerForReuse(inView: collectionView ?? UICollectionView())
    }
    ...
    private func didTapDelete() {
        if Reachability.instance.networkConnectionStatus() == .notConnection {
            showAlertView(ChatLoginConstant.checkInternet, message: ChatLoginConstant.checkInternetMessage)
            SVProgressHUD.dismiss()
            return
        }
